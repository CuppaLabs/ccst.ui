            ...
            $http.get('/api/v1/graph_scc?project_id=' + $scope.projectID)
                .then(function (response) {
                    for (var i = 0; i < response.data.SCC.length; i++) {
                        ...
                        if (response.data.SCC[i] == null || (response.data.SCC[i].PUBLICSCORE == null && response.data.SCC[i].PRIVATESCORE == null)) {

                            $scope.feed[i].PUBLICSCORE = 0.00;
                            $scope.feed[i].PRIVATESCORE = 0.00;
                            $scope.feed[i].APPLICATIONID =  0;
                        }
                        else {
                            ...
                            $scope.feed[i].APPLICATIONID = response.data.VERTICES[i].APPLICATIONID;
                        }
                    }
                });
        }
        ...
        function prendoValoriReadiness() {
            ...
            $http.get('/api/v1/decision?application_id=' + $scope.applicationID + '&project_id=' + $scope.projectID)
                .then(function(response) {
                    ...
                    if (response.data[0] == null || (response.data[0].PUBLICSCORE == null && response.data[0].PRIVATESCORE == null)) {
