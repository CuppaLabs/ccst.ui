...
(function () {
    ...
    function BlurFeedCtrl($scope, $http, dataService, $state, $rootScope) {
        ...
        $scope.checkCompleted = function(message){

            if ( message.HASALLCRITERIA == 1 || message.HASTEMPLATE == 1) {

                $scope.visibility = true;
            }

            return  message.HASALLCRITERIA == 1 ||
                message.HASTEMPLATE == 1
            ...
            $http.get('/api/v1/decision?application_id=' + $scope.applicationID + '&project_id=' + $scope.projectID)
                .then(function(response) {
                    ...
                    if (response.data[0].PUBLICSCORE == null && response.data[0].PRIVATESCORE == null) {
                        ...
                        dataService.setApplicationPUBLICSCORE(0.00);
                        dataService.setApplicationPRIVATESCORE(0.00);

                    }

                    else {

                        dataService.setApplicationPUBLICSCORE(response.data[0].PUBLICSCORE.toFixed(2));
                        dataService.setApplicationPRIVATESCORE(response.data[0].PRIVATESCORE.toFixed(2));
